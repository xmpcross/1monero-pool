...
let POOL_NONCE_SIZE = 15 + 1; // +1 for old XMR/new TRTL bugs
...
function handleBlockFound(blockHash, submitDiff, blockHeight, blockDiff, minerAddr, walletAddr) {
    ...
    apiInterfaces.poolSwap({}, function() {});
}
/**
 * Handle miner method
 **/
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result) {
            if (error) {
                ...
                let blockFastHash = utils.cnUtil.get_block_id(shareBuffer, cnBlobType)
                    .toString('hex');
                log('info', logSystem,
                    '######################## MONKEY FOUND MONERO BLOCK ########################Block %s found at height %d by miner %s@%s - submit result: %j######################## MONKEY FOUND MONERO BLOCK ########################',
                    ...
                    if (dataBuffer.indexOf('\n') !== -1) {
                        ...
                        for (let i = 0; i < messages.length; i++) {
                            ...
                            try {
                                ...
                                if (e.message) log('warn', logSystem, 'Exception: ' + e.stack);
                            }
                        }
