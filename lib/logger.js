...
// Set severity levels
let severityLevels = ['debug', 'info', 'warn', 'error', 'block'];
...
global.log = function (severity, system, text, data) {
	...
	if (logConsole) {
		if (config.logging.console.colors) {
            let consoleColour = (system === 'daemon' || system === 'childDaemon') ? 'green' : 'white';
            console.log(severityMap[severity](time) + clc[consoleColour].bold(' [' + system + '] ' + formattedMessage));
		} else {
